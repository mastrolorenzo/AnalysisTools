//
// Used to fit the top mass regressed vs. unregressed distributions
// in the analysis note regression validation plot. Script adapted from
// VBF script.
//
// Original author: Nadezda Chernyavskaya
// 
...
//workspaces
	...
	RooRealVar x(xVar,xVar,120,250);
	sprintf(xVar,"roofit_hist_mbb_reg");
	RooDataHist rh(xVar,xVar,RooArgList(x),hist_mbb_reg);
	sprintf(xVar,"yield_signal_VBF_reg");
	TString tmp;
	RooRealVar m("mean_reg","mean_reg",170,0,500);
	RooRealVar s("sigma_reg","sigma_reg",12,0,100);
	RooRealVar width("fwhm_reg","fwhm_reg",25,0,500);
	...
	RooRealVar n("exp_reg","exp_reg",1,-100,100);
	RooRealVar b0("b0_reg","b0_reg",0.5,-1.0,1.);
	RooRealVar b1("b1_reg","b1_reg",0.5,-1.0,1.);
	RooRealVar b2("b2_reg","b2_reg",0.5,-1.0,1.);
	RooRealVar b3("b3_reg","b3_reg",0.5,-1.0,1.);
 /// Bkg part: Bernste[i].Data()n     
	//RooBernstein bkg("signal_bkg_reg", "signal_bkg_reg" ,x,RooArgList(b0,b1,b2));
        ...
        //RooLandau bkg("bkg","bkg",x,ml,sl) ;
        RooRealVar mg("mg","mg",200,0,500) ;
        RooRealVar sg("sg","sg",200,0,1000) ;
        RooGaussian bkg("bkg","bkg",x,mg,sg) ;
...
char nVar2[50], xVar2[50];
	...
	RooRealVar width2("fwhm","fwhm",25,0,500);
	...
	RooRealVar n2("exp","exp",1,-100,100);
	RooRealVar b02("b0","b0",0.5,-1,1.);
	RooRealVar b12("b1","b1",0.5,-1,1.);
	RooRealVar b22("b2","b2",0.5,-1,1.);
	RooRealVar b32("b3","b3",0.5,-1,1.);
 /// Bkg part: Bernste[i].Data()n     
	//RooBernstein bkg2("signal_bkg", "signal_bkg" ,x2,RooArgList(b02,b12,b22));
	 //RooRealVar ml2("ml2","mean landau",5.,-20,20) ;
         ...
         RooRealVar mg2("mg2","mg2",200,0,500) ;
         RooRealVar sg2("sg2","sg2",200,0,1000) ;
         RooGaussian bkg2("bkg2","bkg2",x2,mg2,sg2) ;
 /// Sig part: Crystal Ball   
...
/*	TF1 *ftmp3= (TF1*) model3.asTF(RooArgList(x3),RooArgList(fsig3),RooArgSet(x3));
	...
	TPaveText pave(0.7,0.8,1.-right-top*0.5333,1.-top*1.666,"NDC");
