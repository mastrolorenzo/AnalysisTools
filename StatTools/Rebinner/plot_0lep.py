...
class Plotter(object):
    ...
    GROUPS = {
        'ZH': ['ZH125_ZNuNu_powheg'],
        'ggZH': ['ggZH125_ZNuNu_powheg'],
    ...
    def get_stack(self, bin_edges):
        ...
        histograms['W+Jets'] = self.get_histogram('WJets', bin_edges, 'weight * (bin_index_Znn>-1)')
        histograms['Z+Jets'] = self.get_histogram('ZJets', bin_edges, 'weight * (bin_index_Znn>-1)')
        histograms['WH(b#bar{b})'] = self.get_histogram('WH', bin_edges, 'weight * (bin_index_Znn>-1)')
...
def get_stack_info(stack):
    ...
    for i in xrange(1, n_bins + 1):
        ...
        for hist in stack.GetHists():
            if hist.GetTitle() in {'ZH', 'ggZH', 'WH'}:
...
def main():
    ...
    best_bin_edges = {
        'asimov': [-1, -0.13943498581647873, 0.33239367604255676, 0.6180433332920074, 0.7077441811561584, 0.7889648377895355, 0.7912757098674774, 0.8488226532936096, 0.9013022184371948, 1],
        'poisson': [-1, -0.13943498581647873, 0.33239367604255676, 0.7077441811561584, 0.7889648377895355, 0.7912757098674774, 0.8488226532936096, 0.8515902459621429, 0.9013022184371948, 1],
    }

    best_significance = {
        'asimov': 2.2898901159004432,
        'poisson': 2.5192728275947482,
    }
    ...
    for metric, bin_edges in best_bin_edges.iteritems():
        ...
        with cms_figure.TDRStyle() as style:
            ...
            canvas.SaveAs('bdt_0lep_rebinned_{0}.png'.format(metric))
        ...
        with cms_figure.TDRStyle() as style:
            ...
            canvas.SaveAs('s_over_b_0lep_rebinned_{0}.png'.format(metric))
