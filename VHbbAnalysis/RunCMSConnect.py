...
output = {{ sample }}/$(Process).stdout
error = {{ sample }}/$(Process).stderr
log = {{ sample }}/$(Process).log
...
cd {{ environ['CMSSW_VERSION'] }}/src
eval "$(scramv1 runtime -sh)"

# Setup the AnalysisTools framework.
tar -zxvf "$_CONDOR_SCRATCH_DIR"/AnalysisTools.tar.gz
cd AnalysisTools
source env.sh
make
...
def main(argv=None):
    ...
    if not options.runBatch:
        ...
        scriptFile = os.path.join('condor_runscript.sh')
        ...
        for sample in analysis_manager.samples:
            ...
            submitFile = '%s.submit' % sampleName
