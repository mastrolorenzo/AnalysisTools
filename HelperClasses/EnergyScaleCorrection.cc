  ...
  smearingType_(ECALELF)
{
  ...
  if(!correctionFileName.empty()) {
    ...
    if(scales_.empty()) {
      std::stringstream e("EnergyScaleCorrection");
      e << "scale correction map empty";
  ...
  if(!correctionFileName.empty()) {
    ...
    if(smearings_.empty()) {
      std::stringstream e("EnergyScaleCorrection");
      e << "smearing correction map empty";
      throw std::invalid_argument(e.str());
    }
  }
				    ...
				    unsigned int gainSeed) const
{
  ...
  if(nrFound==0){
    std::cerr << "EnergyScaleCorrection" << "Scale category not found: " << category << " Returning uncorrected value.\n";
    return nullptr;
  }else if(nrFound>1){
    ...
    std::stringstream e("ConfigError");
    e <<" scale error category "<<category<<" has "<<nrFound<<" entries "<<std::endl<<foundCats.str();
    throw std::invalid_argument(e.str());
  }
  //validate the result, just to be sure
  if(!result.first->first.inCategory(runnr,et,eta,r9,gainSeed)){
    std::stringstream e("LogicError");
    e <<" error found scale category "<<result.first->first<<" that does not contain run "<<runnr<<" et "<<et<<" eta "<<eta<<" r9 "<<r9<<" gain seed "<<gainSeed;
    throw std::invalid_argument(e.str());
  }
				    ...
				    unsigned int gainSeed) const
{
  ...
  auto nrFound = std::distance(result.first,result.second);
  if(nrFound==0){
    std::cerr << "EnergyScaleCorrection" << "Smear category not found: " << category << " Returning uncorrected value.\n";
    return nullptr;
  }else if(nrFound>1){
    std::ostringstream foundCats;
    for(auto it = result.first;it!=result.second;++it){
    ...
    std::stringstream e("ConfigError");
    e <<" error smear category "<<category<<" has "<<nrFound<<" entries "<<std::endl<<foundCats.str();
    throw std::invalid_argument(e.str());
  }
  //validate the result, just to be sure
  if(!result.first->first.inCategory(runnr,et,eta,r9,gainSeed)){
    std::stringstream e("LogicError");
    e <<" error found smear category "<<result.first->first<<" that does not contain run "<<runnr<<" et "<<et<<" eta "<<eta<<" r9 "<<r9<<" gain seed "<<gainSeed;
    throw std::invalid_argument(e.str());
  }
  return &result.first->second;
}
				     ...
				     double energyScaleErrSyst, double energyScaleErrGain)
{
  ...
  if(result.first!=result.second){
    std::stringstream e("ConfigError");
    e << "Category already defined! "<<cat;
    throw std::invalid_argument(e.str());
  }
					...
					double eMean, double errEMean)
{
  ...
  if(res.first!=res.second) {
    std::stringstream e("EnergyScaleCorrection");
    e << "Smearing category already defined "<<cat;
    throw std::invalid_argument(e.str());
  }
